...
import { DBTodo, DTOTodo, convertToDBTodo, convertToDTOTodo } from '../../types/DBTodo';
import { v4 } from 'uuid';
...
export const addTodo = async (req: Request, res: Response) => {
  ...
  const todo: DBTodo = convertToDBTodo(req.body?.todo);
  ...
  try {
    ...
    return res.status(200).json(convertToDTOTodo(dbtodo));
...
export const fetchTodos = async (req: Request, res: Response) => {
  ...
  try {
    ...
    return res.status(200).json(todos.map((todo) => convertToDTOTodo(todo)));
  }
  catch (err) {
    console.log(err)
    return handleDBInsertError("cant insert to DB ", res)
